...
class UIController {
  ...
  bindControls() {
    $('play-btn').addEventListener('click', () => this.sim.play());
    $('power-btn').addEventListener('click', () => this.sim.power());
    $('home-btn').addEventListener('click', () => this.sim.home());
    $('pen-btn').addEventListener('click', () => this.sim.togglePen());
    $('engineering').addEventListener('change', e => this.sim.engineering = e.target.checked);
    $('direction').addEventListener('change', () => this.applyInputs());
  ...
  readConfig() {
    const finite = id => { const value = Number($(id).value); return Number.isFinite(value) && $(id).value !== '' ? value : Number($(id).defaultValue); };
    const integer = (id, min, max) => Math.max(min, Math.min(max, Math.round(finite(id))));
    const number = (id, min, max) => Math.max(min, Math.min(max, finite(id)));
    return {
  ...
  refreshStatic() {
    ...
    $('paper-direction').textContent = gears.paperRatio * config.direction < 0 ? 'CCW' : 'CW';
    $('arm-a-direction').textContent = gears.armARatio * config.direction < 0 ? 'CCW' : 'CW';
    $('arm-b-direction').textContent = gears.armBRatio * config.direction < 0 ? 'CCW' : 'CW';
  ...
  render() {
    ...
    $('machine-message').textContent = sim.message;
    $('power-btn').textContent = sim.powered ? 'Power off' : 'Power on';
    $('home-btn').disabled = !sim.powered || sim.homing;
    $('play-btn').disabled = !sim.powered || !sim.homed || sim.homing || !sim.linkageValid;
    $('pen-btn').disabled = !sim.powered || !sim.homed || sim.homing;
    $('pen-btn').textContent = sim.penDown ? 'PEN DOWN · lift' : 'PEN UP · lower';
    ...
    $('data-driver-rpm').textContent = (sim.running ? sim.config.direction * sim.config.driverRPM : 0).toFixed(2);
    $('data-paper-rpm').textContent = minus(((sim.running ? sim.config.direction * sim.config.driverRPM : 0) * sim.gears.paperRatio).toFixed(2));
    $('data-arm-a-rpm').textContent = minus(((sim.running ? sim.config.direction * sim.config.driverRPM : 0) * sim.gears.armARatio).toFixed(2));
    $('data-arm-b-rpm').textContent = minus(((sim.running ? sim.config.direction * sim.config.driverRPM : 0) * sim.gears.armBRatio).toFixed(2));
